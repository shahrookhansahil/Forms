	...
	return (
		<Formik<FormValues>
			initialValues={{
				...
				shift: "",
				date: "",
				auditItems: [{ concern: "", isolationTag: "", jobTag: "" }],
				reaction: "",
				pacCru: "",
				solvent_recovery: "",
				to_field: "",
							...
							<div className="space-y-4 border border-gray-300 rounded-md p-2">
								...
								{values.auditItems.map((item, index) => (
									<div key={index} className="flex items-center gap-4 ">
										...
										<div className="w-3/12">
											<Field
												type="text"
												name={`auditItems[${index}].isolationTag`}
												placeholder="Isolation Tag"
												className="p-2 border border-gray-300 rounded-md w-full"
											/>
											{touched.auditItems &&
												touched.auditItems[index] &&
												errors.auditItems?.[index]?.isolationTag && (
													<div className="text-sm text-red-500">
														{errors.auditItems[index].isolationTag}
													</div>
												)}
										...
										<div className="w-3/12">
											<Field
												type="text"
												name={`auditItems[${index}].jobTag`}
												placeholder="Job Tag"
												className="p-2 border border-gray-300 rounded-md w-full"
											/>
											{touched.auditItems &&
												touched.auditItems[index] &&
												errors.auditItems?.[index]?.jobTag && (
													<div className="text-sm text-red-500">
														{errors.auditItems[index].jobTag}
													</div>
												)}
								...
								<div className="flex justify-start ">
									<button
										type="button"
										onClick={() =>
											push({ concern: "", isolationTag: "", jobTag: "" })
										}
